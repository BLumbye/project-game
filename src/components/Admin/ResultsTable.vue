...
<script setup
        ...
        const columns = [
          ...
          ...Object.entries(config.events).filter(([eventName, event]) => event.choices !== undefined).map(([eventName, event]) => columnHelper.accessor(row => row.eventChoices![eventName] || 'Undecided', {
            id: `event-${eventName}`,
            cell: info => info.getValue(),
