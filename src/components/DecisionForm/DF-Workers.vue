...
<template>
  <div class="workers">
    ...
    <template v-for="(worker, key) in config.workers"
              :key="key">
      <label :for="`${key}-input`"
             class="worker-label">{{ capitalize(worker.label) }}</label>
      <input v-model="inputs[key]"
             type="text"
...
<script setup
        ...
        const inputError = computed(() => (worker: string) => workersStore.currentWorkers[worker] + Number(inputs.value[worker]) < 0);
        ...
        watch(
          ...
          () => {
            for (const key in inputs.value) {
              inputs.value[key] = workersStore.workers[gameStore.week][key] !== 0 ? workersStore.workers[gameStore.week][key].toString() : '';
            }
          }, { immediate: true }
        );
</script>
