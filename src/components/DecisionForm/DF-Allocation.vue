  ...
  <div class="allocation">
    ...
    <div class="component-title">
      ...
      <v-tooltip activator="parent" location="top">
        <p>
          Remember to allocate workers again in every {{ gameStore.config.durationIdentifier.singular }}'s 'Decision
          Form'.
        </p>
    ...
    <span
      ...
      :key="key"
      v-tooltip:top="{
        text: `Because you are trying to use more ${worker.plural} than you have hired some activities will not progress.`,
    ...
    <template v-for="(activity, index) in activityStore.activities" :key="activity.label">
      <template v-if="!activityStore.isActivityHidden(activity)">
        ...
        <input
          ...
          v-tooltip:top="{
            text: `Because you are trying to use more ${worker.plural} than you have hired this activity will not progress.`,
...
const activities = gameStore.config.activities;
...
watch(
  ...
  () => {
    activityStore.activities.forEach((activity, i) => {
      for (const worker in gameStore.config.workers) {
