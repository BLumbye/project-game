...
<template>
  <template v-if="!answered">
    <form @submit.prevent="handleSubmit">
      <template v-for="(SectionComponent, i) in activeSections"
                :key="i">
        <component :is="SectionComponent"
                   :show="currentSection === i"
                   ref="sectionRefs" />
      </template>
      <div class="buttons">
        <button class="text-button"
                :disabled="currentSection === 0"
                @click="currentSection--">Back</button>
        <input type="submit"
               :value="currentSection === (activeSections.length - 1) ? 'Submit' : 'Continue'" />
      </div>
    </form>
  </template>
  <template v-else>
...
<script setup
        ...
        const handleSubmit = (evt: Event) => {
          ...
          if (!currentSectionComponent || currentSectionComponent.errorCheck())
            return;
